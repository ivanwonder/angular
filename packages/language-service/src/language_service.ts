  ...
  getSuggestionDiagnostics(fileName: string): ts.DiagnosticWithLocation[] {
    return this.withCompilerAndPerfTracing(PerfPhase.LsSuggestionDiagnostics, (compiler) => {
      ...
      if (isTypeScriptFile(fileName)) {
        ...
        const sourceFile = program.getSourceFile(fileName);
        if (sourceFile) {
          const ngDiagnostics = compiler.getTemplateTypeChecker().getSuggestionDiagnosticsForFile(
              sourceFile, OptimizeFor.SingleFile);
          diagnostics.push(...filterNgDiagnosticsForFile(ngDiagnostics, sourceFile.fileName));
        }
      } else {
        const components = compiler.getComponentsWithTemplateFile(fileName);
        for (const component of components) {
          if (ts.isClassDeclaration(component)) {
            diagnostics.push(
                ...compiler.getTemplateTypeChecker().getSuggestionDiagnosticsForComponent(
                    component));
          }
        }
      }
